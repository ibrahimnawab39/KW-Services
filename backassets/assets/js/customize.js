$("#add").on("submit", function (e) {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      }else if (result['res'] == "success1") {
        $("#alert").html(
          '<div class="alert alert-success inverse alert-dismissible fade show" role="alert"><p>Your withdrawal request send to admin.</p></div>'
        );
        $("#add").trigger("reset");
        $("html, body").animate(
          {
      ...
      } else if (result['res'] == "limitexit") {
        $("#alert").html(
          '<div class="alert alert-info inverse alert-dismissible fade show" role="alert"><p>Your Image Limit Is Exceed</p></button></div>'
        );
        $("html, body").animate(
          {
            scrollTop: 0,
...
$("#update").on("submit", function (e) {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result['res'] == "success") {
        $("#alert").html(
          '<div class="alert alert-success inverse alert-dismissible fade show" role="alert"><p><b> Well done! </b>Successfully Updated.</p></div>'
        );
...
$('.refresh0').html(result);
  ...
  $("#login").on('submit', function(e) {
    ...
    $('.btn-sbmit').attr('disabled', 'disabled');
    $.ajax({
      url: 'include/fetch.php?page=login',
      type: "POST",
      data: new FormData(this),
      contentType: false,
      processData: false,
